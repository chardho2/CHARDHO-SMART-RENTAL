<?php


// Security file
defined('BASEPATH') OR exit('Not found');

class Auth extends CI_Controller{

    function __construct(){
        parent::__construct();
          // Load Model
        $this->load->model('Auth_Model');

         //Language
        $this->config->set_item('language', $this->expressgo->setup("language"));
        $this->lang->load('expressgo', $this->expressgo->setup("language"));
        
    }

    /**
     * Login
     */
    public function login(){


        $data['alert']=NULL;
        // Post data
        if(isset($_POST['loginF'])){
           $_login   = $this->input->post('login');
           $password = $this->input->post('password');

         //check login data
           $login = $this->Auth_Model->login($_login,$password);
           if($login){
            // Save session
            $this->session->set_userdata('username',$_login);
            $this->session->set_userdata('password',$password);
            $this->session->set_userdata('login',TRUE);
            redirect('/dashboard');
        }else{
            $data['alert']='<br/><div class="alert alert-danger">
            <span><b> '.$this->lang->line('error').' - </b> '.$this->lang->line('login_password_wrong').'</span>
            </div>';
        }
    }

    $this->load->view('expressgo/auth/login_view',$data);
}

    /**
     * Logout
     */
    public function logout(){
        $this->session->unset_userdata('username');
        $this->session->unset_userdata('password');
        $this->session->unset_userdata('login');
        redirect('/login');
    }




    /**
     * Password change
     */
    public function profile(){

       //If Script installed, check login
     if($this->expressgo->setup("install") == 1){
        if(!$this->session->userdata('login')){redirect('/login');}
    }


    $data['title'] = $this->lang->line('profile');
    $data['alert']=NULL;
    $data['js_code']=NULL;
    $this->load->view('expressgo/static/header_view',$data);

        //Post data
    if ($this->input->post()){

        $new_password = $this->input->post("new_password");
        $confirm_password = $this->input->post("confirm_password");
                //Check new password
        if(empty($new_password)){
            $data['alert']='<br/><div class="alert alert-danger">
            <span><b> '.$this->lang->line('error').' - </b> '.$this->lang->line('password_not_empty').'</span>
            </div>';
        }elseif($new_password == $confirm_password){

            $parameters = array(
                'password'=> $this->input->post('new_password')
            );
            $this->Auth_Model->update($parameters);

            $this->session->set_userdata('password',$this->input->post('new_password'));

            if($this->expressgo->setup("install") == 1){
                $data['alert']='<br/><div class="alert alert-success">
                <span><b> '.$this->lang->line('successful').' - </b> '.$this->lang->line('password_changed').'</span>
                </div>';
            }else{
                $this->Auth_Model->s_update('setup',array('value'=> 1),'install','parameter');
                redirect(site_url('/login'));
            }

            
        }else{
            $data['alert']='<br/><div class="alert alert-danger">
            <span><b> '.$this->lang->line('error').' - </b> '.$this->lang->line('password_not_same').'</span>
            </div>';
        }


    }



    $this->load->view('expressgo/auth/profile',$data);
    $this->load->view('expressgo/static/footer_view');
}

    /**
     * Google OAuth Callback
     * Receives the Google ID token (JWT) POSTed from the login page's handleGoogleCredential(),
     * decodes + verifies it, then creates a CodeIgniter session.
     */
    public function google_callback() {

        // Only accept POST requests
        if ($this->input->server('REQUEST_METHOD') !== 'POST') {
            redirect('/login');
            return;
        }

        $credential = $this->input->post('credential');
        if (empty($credential)) {
            redirect('/login');
            return;
        }

        // ── Decode the JWT payload (header.payload.signature) ──
        $parts = explode('.', $credential);
        if (count($parts) !== 3) {
            redirect('/login');
            return;
        }

        $payload_raw = $parts[1];
        $payload_raw = str_replace(['-', '_'], ['+', '/'], $payload_raw);
        $pad = strlen($payload_raw) % 4;
        if ($pad) $payload_raw .= str_repeat('=', 4 - $pad);
        $payload = json_decode(base64_decode($payload_raw), true);

        if (!$payload) {
            redirect('/login');
            return;
        }

        // ── Security checks ──
        // 1. Audience must match our Google Client ID
        if (!defined('GOOGLE_CLIENT_ID') || $payload['aud'] !== GOOGLE_CLIENT_ID) {
            show_error('Google Sign-In failed: Client ID mismatch. Please set GOOGLE_CLIENT_ID in application/config/constants.php');
            return;
        }

        // 2. Token must not be expired
        if (isset($payload['exp']) && $payload['exp'] < time()) {
            show_error('Google Sign-In failed: Token expired. Please try again.');
            return;
        }

        // 3. Issuer must be Google
        $valid_issuers = ['accounts.google.com', 'https://accounts.google.com'];
        if (!in_array($payload['iss'], $valid_issuers)) {
            show_error('Google Sign-In failed: Invalid token issuer.');
            return;
        }

        // ── Extract user info ──
        $google_email = isset($payload['email']) ? $payload['email'] : '';
        $google_name  = isset($payload['name'])  ? $payload['name']  : 'Google User';
        $google_pic   = isset($payload['picture']) ? $payload['picture'] : '';

        // Optional: restrict to a specific domain
        // if (!str_ends_with($google_email, '@chardho.com')) { show_error('Only @chardho.com accounts allowed.'); return; }

        // ── Create session ──
        $this->session->set_userdata('username',    $google_email);
        $this->session->set_userdata('google_name', $google_name);
        $this->session->set_userdata('google_pic',  $google_pic);
        $this->session->set_userdata('login',        TRUE);
        $this->session->set_userdata('auth_method', 'google');

        redirect('/dashboard');
    }

}

?>