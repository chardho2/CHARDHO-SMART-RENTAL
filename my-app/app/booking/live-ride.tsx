...
export default function LiveRide() {
    ...
    const handlePaymentSubmit = async (method: string) => {
        try {
            // Check if method is online (PhonePe/GPay/Paytm)
            if (method !== 'cash') {
                Alert.alert(
                    'Payment Successful',
                    `Paid ₹${booking?.fare?.total} via ${method === 'gpay' ? 'Google Pay' : method === 'phonepe' ? 'PhonePe' : 'Paytm'}`
                );
            }

            // Close Payment, Open Rating
            setShowPaymentModal(false);
            setTimeout(() => setShowRatingModal(true), 500);

            // Optionally notify backend about payment method if needed
            // await bookingAPI.updatePayment(booking._id, method);
        } catch (error) {
